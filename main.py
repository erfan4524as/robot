...
def send_tracking(chat_id):
    ...
    for o in reversed(orders[-20:]):
        ...
        if st == "done":
            ...
            st_icon = "⏳ در حال انجام"
        elif st == "pending_payment":
            st_icon = "💳 در انتظار پرداخت"
        else:
            st_icon = "❓ نامشخص"
...
def handle_admin(chat_id, text, data, states):
    ...
    if text == "📨 پیام همگانی":
        ...
        send(chat_id, "*📨 متن پیام را ارسال کنید:*", kb(["❌ لغو"]))
        return
    ...
    if text == "📊 آمار کلی":
        st = data["stats"]
        send(chat_id, f"*📊:\n👥 {st['total_users']:,}\n📦 {st['total_orders']:,}\n💰 {st['total_revenue']:,} تومان*", BACK_BTN)
        return
    if text == "📅 آمار امروز":
                ...
                if "callback_query" in upd:
                    ...
                    if cb_data == "check_join":
                        ...
                        if not t:
                            ...
                            continue
                        if t.get("status") == "closed":
                            send(chat_id, f"⚠️ تیکت #{tid} بسته است.*")
                            continue
